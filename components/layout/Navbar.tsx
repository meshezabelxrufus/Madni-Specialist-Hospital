 ...
 *
 * Behavior:
 *  - Transparent at top (renders over the intro video)
 *  - Frosts (glassmorphism) once user scrolls past 80px
 *  - Nav links scroll to scene start positions via Lenis (px-based)
 *    because the cinematic canvas has no DOM section IDs — the scenes
 *    live at fixed vh offsets defined in scenes.config.ts.
 *  - Book Appointment opens WhatsApp directly
 */
...
import { Heart, Stethoscope, Bone, Scissors } from 'lucide-react'
...
/**
 * vhStart values match the scene start positions in scenes.config.ts.
 * Lenis scrollTo(pixels) is used because the sticky canvas has no DOM IDs.
 */
const NAV_ITEMS = [
  { id: 'pediatrics',  label: 'Pediatrics',  icon: Heart,        vhStart: 280  },
  { id: 'surgery',     label: 'Surgery',     icon: Scissors,     vhStart: 400  },
  { id: 'ent',         label: 'ENT',         icon: Stethoscope,  vhStart: 620  },
  { id: 'orthopedics', label: 'Orthopedics', icon: Bone,         vhStart: 840  },
]
...
export function Navbar() {
  ...
  // On sub-pages the navbar is always solid (no transparent-over-video state)
  ...
  // Scroll to the scene's vh start position via Lenis
  const handleNavClick = (vhStart: number) => {
    scrollTo((vhStart / 100) * window.innerHeight, { duration: 1.6 })
  }

  // On the home page: smooth-scroll to top via Lenis instead of navigating.
  // On any other page: let the <Link> navigate normally (no preventDefault).
    ...
    >
      <div className={styles.inner}>
        ...
        <div className={styles.right}>
          ...
          {/* WhatsApp booking CTA — visually separate from nav links */}
          <button
